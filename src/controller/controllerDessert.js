...
// l'export getData permet d'afficher les données contenu dans le fichier  dessert.json en json dan sla requete
...
exports.getData = (request, response) => {
  ...
  fs.readFile("./src/model/menu.json", (err, data) => {
    ...
    if (err) {
      ...
      response.status(200).json(JSON.parse(data).dessert);
    }
  });
};
...
exports.getDataById = (request, response) => {
  ...
  fs.readFile("./src/model/menu.json", (err, data) => {
    ...
    if (err) {
      ...
      const dataById = manipData.dessert.find(
...
exports.createData = (request, response) => {
  ...
  fs.readFile("./src/model/menu.json", (err, data) => {
    ...
    if (err) {
      ...
      existingData.dessert.push(request.body);
